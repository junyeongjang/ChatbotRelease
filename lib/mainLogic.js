...
exports.mainLogic = async (bot, msg, ChatbotData) =>{//키워드를 찾는 mainlogic
    try{
        const botData = await ChatbotData.findAll({
                     attributes:['data']
        });
        const data = await JSON.parse(botData[1].data); //data[0]은 첫번 째 유저, 0에 유저 아이디 필요
        const contents = await findKeyword(data, msg.text);
